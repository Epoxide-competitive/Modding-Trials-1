...
public class ContainerWand extends Container {
    ...
    public ContainerWand(EntityPlayer player, World world) {
        ...
        this.world = world;
        for (int i = 0; i < 6; i++) {
            final double f = i / 3.0 * Math.PI - 6 * 8;
            this.addSlotToContainer(new Slot(this.inventory, i, (int) Math.sin(f) - 8, (int) Math.cos(f) - 8));
        }
        this.addSlotToContainer(new Slot(this.inventory, 6, 0, 0));
